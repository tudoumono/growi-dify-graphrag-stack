...
# または直接エディタで編集
# graphrag/.env の GROWI_API_KEY= の行に発行したキーを入力
```
...
curl -s -X POST http://localhost:8080/ingest-growi \
  ...
  -d '{"page_id": "<GROWIのページID>", "category": "テスト"}'
```

> ページIDは GROWI のページ URL や管理画面で確認できます。
> `graphrag/.env` に `GROWI_URL` と `GROWI_API_KEY` が設定されている必要があります（手順3を参照）。
...
# その後 API で再取り込み
curl -s -X POST http://localhost:8080/ingest-dir
# 完了状況は GET /ingest-job/{job_id} で確認
```
